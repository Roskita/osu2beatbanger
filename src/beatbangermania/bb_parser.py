...
def find_act_cfg(mod_root: Path) -> Path:
    """act.cfg lives directly at the mod's root,
    fall back to a search if handed a folder that isn't quite the
    root"""
...
def find_levels(mod_root: Path) -> list[Path]:
    """A mod can contain more than one (multiple acts/levels bundled together); 
    each becomes its own .osz on the way back out"""
...
def _note_from_dict(d: dict) -> Note:
   ...
   # note_modifier==3 + "hold_end_timestamp" isn't a valid bb format, but is a placeholder
